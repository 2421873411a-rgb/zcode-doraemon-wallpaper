    <script>
      ...
      (function () {
        ...
        // ★ 保底快捷键（如果 panel.js 加载失败，至少能打开面板）
        document.addEventListener('keydown', function (e) {
          if (!(e.ctrlKey && e.shiftKey && (e.key === 'W' || e.key === 'w'))) return;
          // 如果 panel.js 已经处理了（创建了 dw-theme-panel），不重复
          var existing = document.getElementById('dw-theme-panel');
          if (existing) {
          ...
          p.id = 'dw-theme-panel-fallback';
          p.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:2147483647;padding:20px;background:rgba(15,20,35,0.95);color:#fff;border-radius:14px;font-family:system-ui;min-width:280px;font-size:13px;box-shadow:0 12px 40px rgba(0,0,0,0.5);';
          var themes = _themes || window.__DW_THEMES__ || {};
          var ids = Object.keys(themes).filter(function(k){return k!=='__default__';});
          var active = getActiveTheme();
          var html = '<div style="font-weight:600;margin-bottom:8px;">🎨 主题（保底面板）</div>';
          html += '<div style="max-height:300px;overflow-y:auto;">';
          ids.forEach(function(tid){
            var t = themes[tid];
            var isActive = tid === active;
            html += '<div class="dw-fb-item" data-id="'+tid+'" style="padding:6px 8px;margin:2px 0;border-radius:6px;cursor:pointer;background:'+(isActive?'rgba(126,182,255,0.25)':'rgba(255,255,255,0.06)')+';">'+(isActive?'● ':'○ ')+ (t?t.name:tid) + ' (' + (t?t.type:'?') + ')</div>';
          });
          html += '</div>';
          html += '<div style="font-size:10px;opacity:0.4;margin-top:8px;text-align:center;">快捷键: Ctrl+Shift+W · Esc关闭</div>';
          p.innerHTML = html;
          document.body.appendChild(p);
          
          // 点击切换
          p.querySelectorAll('.dw-fb-item').forEach(function(item){
            item.onclick = function(){
              var tid = item.dataset.id;
              if (window.__dwSwitchTheme) window.__dwSwitchTheme(tid);
